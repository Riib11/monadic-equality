...
{-@ assume
extensionality :: sing_b:EqualPropSing b -> sing_a_to_b:EqualPropSing (a -> b) -> f:(a -> b) -> g:(a -> b) -> (x:a -> EqualProp {sing_b} {f x} {g x}) -> EqualProp {sing_a_to_b} {f} {g}
@-}
extensionality :: EqualPropSing b -> EqualPropSing (a -> b) -> (a -> b) -> (a -> b) -> (a -> Proof) -> Proof
extensionality _ _ _ _ _ = ()

{-@
retractability :: sing_b:EqualPropSing b -> sing_a_to_b:EqualPropSing (a -> b) -> f:(a -> b) -> g:(a -> b) -> EqualProp {sing_a_to_b} {f} {g} -> x:a -> EqualProp {sing_b} {f x} {g x}
@-}
...
class Concreteness a where
  concreteness :: sing:EqualPropSing a -> x:a -> y:a -> EqualProp {sing} {x} {y} -> {_:Proof | x = y}
@-}
class Concreteness a where
...
{-@ assume
concreteness_EqSMT :: EqSMT a => sing:EqualPropSing a -> x:a -> y:a -> EqualProp {sing} {x} {y} -> {_:Proof | x = y}
@-}
...
{-@ assume
reflexivity :: sing:EqualPropSing a -> x:a -> EqualProp {sing} {x} {x}
@-}
...
class Symmetry a where
  symmetry :: sing:EqualPropSing a -> x:a -> y:a -> EqualProp {sing} {x} {y} -> EqualProp {sing} {y} {x}
@-}
class Symmetry a where
...
class Transitivity a where
  transitivity :: sing:EqualPropSing a -> x:a -> y:a -> z:a -> EqualProp {sing} {x} {y} -> EqualProp {sing} {y} {z} -> EqualProp  {sing} {x} {z}
@-}
class Transitivity a where
...
{-@ assume
substitutability :: sing_a:EqualPropSing a -> sing_b:EqualPropSing b -> f:(a -> b) -> x:a -> y:a -> EqualProp {sing_a} {x} {y} -> EqualProp {sing_b} {f x} {f y}
@-}
...
{-@
alphaEquivalency :: (EqualPropSingI b) => sing:EqualPropSing (a -> b) -> f:(a -> b) -> EqualProp {sing} {f} {apply (\x:a -> f x)}
@-}
alphaEquivalency :: (EqualPropSingI b) => EqualPropSing (a -> b) -> (a -> b) -> Proof
alphaEquivalency sing_a_to_b f =
  extensionality (sing :: EqualPropSing b) sing_a_to_b f (apply (\x -> f x)) $ \y ->
    reflexivity (sing :: EqualPropSing b) (f y)
      ? apply (\x -> f x) y

-- where
--   sing_b = sing :: EqualPropSing b
