...
TODO: does this cause problems because it removes the explicit type index in
witnessed propositional equality?
-}
...
instance Concreteness a => Symmetry a where
  symmetry x y = reflexivity x ? concreteness x y

{-
TODO: liquid type error
    ...
    .
    |
112 | instance Symmetry b => Symmetry (a -> b) where
    |          ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
-}
-- instance Symmetry b => Symmetry (a -> b) where
--   symmetry f g =
--     extensionality g f (\x -> symmetry (f x) (g x ? retractability f g x))
